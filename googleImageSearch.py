# SOURCE (modified): http://sam.aiki.info/b/google-images.py
...
def google_image_search_simple(query, n):
    opts = Options()
    opts.add_argument("--headless")
    with webdriver.Chrome(ChromeDriverManager().install(), options=opts) as wd:
        sources = google_image_search(wd, query, safe='off', n=n, opts='', out=sys.stdout)
    return sources

def google_image_search_download(query, n):
    # * create directory for downloaded images and ensure its empty *
    downloads_dir = os.path.join(os.getcwd(), 'downloads')
    # empty directory in case it existed already
    shutil.rmtree(downloads_dir)
    os.mkdir(downloads_dir)

    # do a google search for image urls
    image_urls = google_image_search_simple(query, n)

    # download image urls
    images = []
    for i, image_url in enumerate(image_urls):
        print(f'Downloading... {image_url} ({i+1}/{len(image_urls)})')
        r = requests.get(image_url)  # create HTTP response object
        download_name = f'{i}.png'
        download_path = os.path.join(downloads_dir, download_name)
        with open(download_path, 'wb') as f:
            f.write(r.content)
            images.append(download_path)

    # return list of paths to download image files.
    return images

if __name__ == '__main__':
    #main()
    google_image_search_download('squirrel', 10)